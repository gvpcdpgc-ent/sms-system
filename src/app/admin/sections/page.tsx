...
export default function SectionsPage() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (res.ok) {
                setStatus({ type: "success", message: "Section created successfully!" });
                setName("");
                fetchSections();
                setTimeout(() => {
                ...
                setStatus({ type: "error", message: "Failed to create section." });
            }
        } catch (e) {
    ...
    return (
        <div className="mx-auto max-w-7xl">
            ...
            <div className="grid gap-4 sm:grid-cols-3 lg:grid-cols-4">
                {sections.map((sec) => (
                    <div key={sec.id} className="flex items-center justify-between rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <p className="text-xs text-slate-400">ID: {sec.id.substring(0, 8)}...</p>
                            </div>
                        </div>
                    </div>
                ))}
